...
llm = get_langchain_model(temperature=0, max_completion_tokens=2048)

checkpoint = InMemorySaver()

# Get API keys from environment variables
TAVILY_SEARCH_API = os.getenv(
    "TAVILY_API_KEY", "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"
)
...
def query_node(state: AgentState):
    ...
    return {"queries": response.queries}  # type: ignore
...
def revision_node(state: AgentState):
    ...
    messages = [
        ...
        HumanMessage(
            content="Por favor, revise o planejamento com base no feedback fornecido e nas informações disponíveis."
        ),
    ]
...
builder.add_conditional_edges(
    "feedback", should_continue, {"feedback": "revision", "end": END}
)
...
def display_graph():
    ...
    try:
        ...
        print(
            "This is optional and doesn't affect the functionality of the travel planner."
        )
...
def run_travel_planner():
    ...
    for step in graph.stream(initial_state, thread):
        if "query" in step:
            ...
            print(
                "✅ Plano revisado com base no seu feedback e informações da pesquisa!"
            )
